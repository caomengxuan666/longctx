...
- `context.index.json`: Context-level routing index generated by `longctx generate` or `longctx index`.
...
- Ambiguous routing fails the row with `ContextRoute` so benchmark results remain auditable.
